...
export default function Coaches() {
    ...
    useEffect(() => {
        async function fetchCoachesList() {
            const usersList = await UserDatabaseService.fetchAllUsers();
            if (!Array.isArray(usersList)) return;

            const coaches = usersList.filter(user => user.isCoach);
        ...
        async function fetchUserApplicationsList() {
            const applications = await ApplicationDatabaseService.fetchUserApplications(user.id);
    ...
    function canHireCoach(coachId) {
        const hasApplication = Array.isArray(userApplications) &&
            userApplications.some(app => app.coachId === coachId && (app.status === "pending" || app.status === "active"));
        const hasAffiliation = Array.isArray(userAffiliations) &&
    ...
    function handleHire(coachId) {
        if ((userAffiliations && Array.isArray(userAffiliations)) || (userApplications && Array.isArray(userApplications))) {
            ...
            if (hasAffiliationWithCoach || hasApplicationWithCoach) {
                createToast({ message: "You are already affiliated with this coach or have an active application with them." });
                return;
            }
        }
        setSelectedCoach(coachId);
    ...
    async function initRequest(coachId, message) {
        ...
        try {
            showSpinner();
            await ApplicationDatabaseService.createApplication(requestData);
            hideSpinner();
            createAlert({ title: "Success", text: `Your request has been sent successfully!\nYou will be notified once they respond.` });
            if (userApplications && Array.isArray(userApplications))
                setUserApplications(prev => [...prev, requestData]);
            else
                setUserApplications([requestData]);
        } catch (error) {
    ...
    function renderCoachItem(coach) {
        ...
        const rotate = arrowAnimations[coach.id]?.interpolate({
            inputRange: [0, 1],
            ...
            >
                <View style={styles.applicationItemDetails}>
                    ...
                    <View style={styles.detailsContainer}>
                        <Text allowFontScaling={false} style={styles.detail}>{coach.firstname} {coach.lastname}</Text>
                        <Text allowFontScaling={false} style={styles.detail}>{coach.age}, {coach.gender}</Text>
                    </View>
                ...
                {selectedCoach === coach.id && (
                    <View style={{ marginTop: 10 }}>
                        <View style={styles.detailsContainer}>
                            <Text allowFontScaling={false} style={styles.detail}>○ {coach.email}</Text>
                            <Text allowFontScaling={false} style={styles.detail}>○ {coach.phone}</Text>
                        </View>
                        <AnimatedButton
        ...
        <View style={styles.main}>
            ...
            {coachesList && coachesList.length > 0 ? (
                <>
                    ...
                    >
                        <View style={styles.innerHeaderContainer}>
                            <Text allowFontScaling={false} style={styles.headerText}>Available Coaches</Text>
                    ...
                    <View style={{ width: '100%', backgroundColor: 'rgb(12,12,12)', padding: 10, zIndex: 10 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                            <TouchableOpacity disabled={currentPage === 1} onPress={() => setCurrentPage(p => p - 1)}>
                                <Text style={{ color: currentPage === 1 ? 'gray' : 'white', padding: 10 }}>Prev</Text>
                            </TouchableOpacity>

                            <Text style={{ color: 'white', padding: 10 }}>Page {currentPage}</Text>

                            <TouchableOpacity
                                disabled={currentPage === maxPage}
                                onPress={() => currentPage < maxPage && setCurrentPage(p => p + 1)}
                            >
                                <Text style={{ color: currentPage === maxPage ? 'gray' : 'white', padding: 10 }}>Next</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                            {["all", "canApply"].map(option => (
                                ...
                                >
                                    <Text style={{
                                        color: filterCanApply === option ? 'rgb(0,140,255)' : 'white',
                                        fontWeight: 'bold',
                                        padding: 10,
                                    }}>
                ...
                <View style={styles.noAffiliatesView}>
                    ...
                    <Text allowFontScaling={false} style={styles.noAffiliatesText}>Empty, no coaches available...</Text>
                </View>
            )}
        </View>
    );
