...
export default function Settings() {
    ...
    function revertBackHandler() {
        setBackHandler(() => { router.back(); return true; });
    };
    ...
    async function setNextLanguage() {
        const next = language === 'English' ? 'Arabic' : language === 'Arabic' ? 'Hebrew' : 'English';
        setLanguage(next);
        await AsyncStorage.setItem("userPreferences", JSON.stringify({
            ...user.preferences,
            language: next,
    ...
    function initDeleteHandler() {
        createInput({
            ...
            confirmText: "DELETE",
            confirmButtonStyle: { color: 'rgb(255,59,48)' },
            text: `Please enter your password to confirm`,
            placeholder: `Password`,
            onCancel: () => revertBackHandler(),
            onSubmit: (password) => {
                if (password !== user.password) {
                    createAlert({ title: "Delete Account", text: "Incorrect password!", onPress: () => revertBackHandler() })
                    return;
                }
                setDeleting(true);
            }
        ...
        <ImageBackground source={theme === 'dark' ? Images.darkProfileBackground : Images.lightProfileBackground} resizeMode="cover" style={styles.background}>
            ...
            <View style={styles.overlay}>
                <View style={styles.topSection}>
                    ...
                    <View style={styles.optionRow}>
                        <Text allowFontScaling={false} style={styles.label}>Language - ({language})</Text>
                        <TouchableOpacity style={styles.modeIcon} onPress={setNextLanguage}>
                            <Image
                                source={language === 'English' ? Images.english : language === 'Arabic' ? Images.arabic : Images.hebrew}
